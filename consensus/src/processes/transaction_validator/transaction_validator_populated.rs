...
use karlsen_consensus_core::{hashing::sighash::SigHashReusedValues, tx::{TransactionInput, VerifiableTransaction},};
...
impl TransactionValidator {
    pub fn validate_populated_transaction_and_get_fee(
            ...
            && pov_daa_score > self.storage_mass_activation_daa_score
        {
            ...
            if pov_daa_score < self.storage_mass_activation_daa_score + 10
                && self.storage_mass_activation_daa_score > 0
            {
    ...
    fn check_transaction_coinbase_maturity(
        ...
        if let Some((index, (input, entry))) =
            tx.populated_inputs().enumerate().find(|(_, (_, entry))| {
                entry.is_coinbase && entry.block_daa_score + self.coinbase_maturity > pov_daa_score
            })
        {
    ...
    fn check_transaction_output_values(
        tx: &impl VerifiableTransaction,
        total_in: u64,
    ) -> TxResult<u64> {
    ...
    fn check_mass_commitment(&self, tx: &impl VerifiableTransaction) -> TxResult<()> {
        ...
        if committed_contextual_mass != calculated_contextual_mass {
            return Err(TxRuleError::WrongMass(
                calculated_contextual_mass,
                committed_contextual_mass,
            ));
        }
        Ok(())
    }
    ...
    fn check_sequence_lock(tx: &impl VerifiableTransaction, pov_daa_score: u64) -> TxResult<()> {
        ...
        if tx
            .populated_inputs()
            .filter(|(input, _)| {
                input.sequence & SEQUENCE_LOCK_TIME_DISABLED != SEQUENCE_LOCK_TIME_DISABLED
            })
            .any(|(input, entry)| {
    ...
    fn check_sig_op_counts<T: VerifiableTransaction>(tx: &T) -> TxResult<()> {
        for (i, (input, entry)) in tx.populated_inputs().enumerate() {
            let calculated =
                get_sig_op_count::<T>(&input.signature_script, &entry.script_public_key);
            if calculated != input.sig_op_count as u64 {
                return Err(TxRuleError::WrongSigOpCount(
                    i,
                    input.sig_op_count as u64,
                    calculated,
                ));
            }
        }
...
mod tests {
    ...
    use karlsen_consensus_core::tx::{
        MutableTransaction, PopulatedTransaction, ScriptVec, TransactionId, UtxoEntry,
    };
    use karlsen_consensus_core::tx::{
    ...
    fn check_signature_test() {
        ...
        let prev_tx_id = TransactionId::from_str(
            "746915c8dfc5e1550eacbe1d87625a105750cf1a65aaddd1baa60f8bcf7e953c",
        )
        .unwrap();
        ...
        faster_hex::hex_decode(
            "20e1d5835e09f3c3dad209debcb7b3bf3fb0e0d9642471f5db36c9ea58338b06beac".as_bytes(),
            &mut bytes,
        )
        .unwrap();
        ...
        let mut bytes = [0u8; 34];
        faster_hex::hex_decode(
            "200749c89953b463d1e186a16a941f9354fa3fff313c391149e47961b95dd4df28ac".as_bytes(),
            &mut bytes,
        )
        .unwrap();
        ...
        tv.check_scripts(&populated_tx)
            .expect("Signature check failed");
    }
    ...
    fn check_incorrect_signature_test() {
        ...
        assert!(
            tv.check_scripts(&populated_tx).is_err(),
            "Failing Signature Test Failed"
        );
    }
    ...
    fn check_multi_signature_test() {
        ...
        faster_hex::hex_decode(
            "aa2071b6c2c604a8830a1484ba469e845c37bb0af32f044bc8fd0c892c8878419e8587".as_bytes(),
            &mut bytes,
        )
        .unwrap();
        let script_pub_key_1 = SmallVec::from(bytes.to_vec());
        ...
        let mut bytes = [0u8; 34];
        faster_hex::hex_decode(
            "206c376f9da440494e18b283803698ed13249af93be3e99f58f42d7d82744d3d15ac".as_bytes(),
            &mut bytes,
        )
        .unwrap();
        let script_pub_key_2 = SmallVec::from(bytes.to_vec());
    ...
    fn check_non_push_only_script_sig_test() {
        ...
        assert!(
            result
                == Err(TxRuleError::SignatureInvalid(
                    TxScriptError::SignatureScriptNotPushOnly
                ))
        );
    }
    ...
    fn test_sign() {
        ...
        assert_eq!(
            TransactionValidator::check_sig_op_counts(&populated_tx),
            Ok(())
        );
    }
}
